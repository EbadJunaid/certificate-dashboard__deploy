...
// Global charts registry to keep track of all created charts
const chartsRegistry = {};

/**
 * Ensure canvas is ready for a new chart by destroying any existing chart
 * @param {string} canvasId - Canvas element ID
 */
function ensureCanvasReady(canvasId) {
  // If a chart exists for this canvas ID, destroy it first
  if (chartsRegistry[canvasId]) {
    chartsRegistry[canvasId].destroy();
    delete chartsRegistry[canvasId];
  }
}
...
function createDoughnutChart(canvasId, labels, data, options = {}) {
  ...
  // Clean up existing chart on this canvas
  ensureCanvasReady(canvasId);
...
function destroyChart(chart) {
  if (chart) {
    ...
    for (const [id, registeredChart] of Object.entries(chartsRegistry)) {
      if (registeredChart === chart) {
...
/**
 * Destroy all charts
 * Useful when changing views
 */
function destroyAllCharts() {
  for (const id in chartsRegistry) {
    if (chartsRegistry[id]) {
      chartsRegistry[id].destroy();
    }
  }
  // Clear the registry
  Object.keys(chartsRegistry).forEach((key) => delete chartsRegistry[key]);
}
